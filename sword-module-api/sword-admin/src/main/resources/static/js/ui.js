...
(function ($) {
    $.extend({
    	...
    	table: {
            ...
            // 导入数据
            importExcel: function(formId, excel) {
            	...
            	layer.open({
            		...
            		btn1: function(index, layero){
            			...
            			if (excel && (file == '' || (!$.common.endWith(file, '.xls') && !$.common.endWith(file, '.xlsx')))){
        ...
        treeTable: {
            ...
            refresh: function(tableId) {
            	var currentId = $.common.isEmpty(tableId) ? table.options.id : tableId;
            	$("#" + currentId).bootstrapTreeTable('refresh');
            },
            // 查询表格树指定列值
            selectColumns: function(column) {
        ...
        operate: {
            ...
            successTabCallback: function(result) {
                if (result.code == web_status.SUCCESS) {
    	            ...
    	            var $contentWindow = $('.kcrab_iframe[data-id="' + currentId + '"]', topWindow)[0].contentWindow;
    	            $.modal.close();
    	...
    	common: {
			...
			displayDate: function(value) {
			    ...
			    return value = h + "小时" + m + "分" + s + "秒";
			},
			addMonths: function(d, m) {
				var d = new Date(d);
			...
			toPercent: function(value, fixed) {
				if (fixed == null || fixed == undefined){
					fixed = 2;
				}
				var str=Number(value*100).toFixed(fixed);
			    str+="%";
			    return str;
			},
