...
public class HttpUtil {
	...
	private static final int CONNECT_TIME_OUT = 10000; // 设置连接超时
	private static final int SOCKET_TIME_OUT = 20000; // 设置读取超时

	public static String post(String url, Map<String, Object> params) throws Exception {
		log.debug("POST: {}", url + "?" + paramToUrl(params));
		String result = null;
		CloseableHttpClient httpClient = SSLClient.createSSLClientDefault();// HttpClientBuilder.create().build();
...
//		} catch (IOException e) {
//			e.printStackTrace();
//			log.debug("IOE: " + e.getLocalizedMessage());
		} catch (Exception e) {
			e.printStackTrace();
	...
	public static String get(String url) throws Exception {
		log.info("GET：{}", url);
		String result = "";
		CloseableHttpClient httpClient = SSLClient.createSSLClientDefault();// createSSLInsecureClient(); //
		...
		try {
			...
			log.error("Error：{} => {}", url, e.getLocalizedMessage());
			throw e;
		} finally {
			try {
				httpClient.close();
